...
read -r -d '' HELP <<EOF
logs test runner. Execute unit test suite and some other details.
...
Options:
    -s <...>
        Specifies which test suite to run
            ...
            - coverage: PHP unit tests with coverage
            - functional: functional tests
            - acceptance: backend acceptance tests
    ...
    -c <port>
        Change the port that the chrome vnc port is mapped to on the host. Default is 19782.
...
TEST_FILE=${1}
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/logs/${1}"
fi
